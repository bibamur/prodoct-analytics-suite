...
"""
event_a_df = all_user_events_df.loc[(all_user_events_df[list(event_a)] == pd.Series(event_a)).all(axis = 1)]
event_a_df['date'] = pd.to_datetime(event_a_df['date'])

len(event_a_df.index)
...
for i in range(delta.days):
    ...
    print(day.date())
    day_events = event_a_df.loc[event_a_df['date'].apply(lambda x: x.date()) == day.date()]
    day_counts_dict = {
            'Event_id':['a'],
            'date': [day.date()],
...
1. Need to have dictionary of events
2. For each event:
    Filter original dataframe with this event
    Calculate count of entries by day
    Calculate count of unique users by day
    Event_ID, Day, count of entries, count of unique users.
